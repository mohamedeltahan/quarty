...
<style>
  ...
  <body>
    ...
    <input value="{{route("orders.update","X")}}" id="itemsupdate" class="d-none">
    <input value="{{route("GetOrderUser")}}" id="GetOrderUser" class="d-none">
      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
                    ...
                    <div class="table-responsive">
                        <div class="table-wrapper">
                            <div class="table-title">
                                <div class="row">
                                    <div class="col-sm-4">
                                    <a href="{{route("orders.index")}}" style="color: white">	<h2>Manage <b>Orders</b></h2> </a>
                                    </div>
                                    <div class="col-sm-4">
                         ...
                         @endif
                            ...
                            <table class="table table-striped table-hover" style="text-align: center"  id="MainTable"> 
                                ...
                                <tbody>
                                    ...
                                    <tr  data-id="{{$order["id"]}}"  data-description="{{$order["details"]}}">
                                        <td class="total_price_before_discount">{{$order["total_price_before_discount"]}}</td>
                                        <td class="total_price_after_discount">{{$order["total_price_after_discount"]}}</td>
                                        <td class="discount">{{$order["discount"]}}</td>
                                        <td class="receiving_date">{{$order["receiving_date"]}}</td>
                                        <td class="dispatching_date">{{$order["dispatching_date"]}}</td>
                                        <td class="state">{{$order["state"]}}</td>
                                        <td class="delivery">{{$order["delivery"]}}</td>
                                        <td><i class="fa fa-list-ol" href="#OrderItemsModal"  data-toggle="modal" aria-hidden="true"></i></td>
                                        <td><i class="fa fa-users" href="#OrderUsersModal"  data-toggle="modal" aria-hidden="true"></i></td>
                            ...
                            <div class="clearfix">
                              <div class="hint-text">Showing <b>{{$orders["per_page"]}}</b> out of <b>{{$orders["total"]}}</b> entries</div>
                              <ul class="pagination">
                                @foreach($orders["links"] as $url)
          ...
          <table id="TempItemTable">
            ...
            <!-- Users Modal --!>
                <div id="OrderUsersModal" class="modal fade">
                      ...
                      <div class="modal-content">
                          <form  method="post" action="{{route("items.store")}}" enctype='multipart/form-data'>
                              ...
                              <div class="modal-body">					
                                  ...
                                  <table class="table table-striped table-hover" style="text-align: center"  id="OrderItemsTable"> 
                                      <thead>
                                          <tr>
                                              <th>name</th>
                                              <th>code</th>
        ...
        if($.ajax({
            ...
            success: function (result) {
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    tr.find(".country").text(result[i].country);
                    tr.find(".governorate").text(result[i].governorate);
                    tr.find(".district").text(result[i].country);
                    tr.find(".street").text(result[i].street);
                    tr.find(".floor").text(result[i].floor);
                    tr.find(".apartment").text(result[i].apartment);
